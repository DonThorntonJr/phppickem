...
?>
	...
	while ($row = $query->fetch_assoc()) {
		...
		echo '			First game: &nbsp; ' . date('Y-m-d - H:i', strtotime($row['firstGameTime'])-(SERVER_TIMEZONE_OFFSET * 3600)) . ' (' . SERVER_TIMEZONE_ABBR . ') <br />' . "\n";
		echo '			Cutoff: &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; ' . date('Y-m-d - H:i', strtotime($row['cutoffTime'])-(SERVER_TIMEZONE_OFFSET * 3600)) . ' (' . SERVER_TIMEZONE_ABBR . ') </p>' . "\n";
		//echo '		</tr>'."\n";
		if ($row['expired']) {
